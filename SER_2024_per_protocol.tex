...
\section{RCTs}
    ...
    \begin{frame}{Per-protocol effects can be biased}
        ...
        \begin{itemize}
            ...
            \item[\ding{43}] Susceptible to selection bias 
        \end{itemize}
    ...
    \vspace{0.4cm}
    $\textbf{L}$ : vector of covariates\\

    Can be addresed: e.g with inverse probability weighting (see next talk)
    \end{column}
\end{columns} 
    \note{
        \begin{itemize}
    ...
    \begin{frame}{There is no \textit{one} per-protocol effect}
        ...
        \begin{column}{0.6\textwidth}
            \begin{itemize}
                ...
                \item There are also $k\in\{1,\ldots,\infty\}$ protocols depend on how the investigator(s) define adherence.
            \end{itemize}
        \end{column}
        \begin{column}{0.4\textwidth}
            $$
...
\end{frame}
    ...
    \begin{frame}{Example: ACTG 5202 Reanalysis}
            Objective:
        \begin{itemize}
            \item Per-protocol analyses modulating protocol definition.
